...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if shouldAutoInjectImageGenerationTool(e.cfg, opts) {
...
func shouldAutoInjectImageGenerationTool(cfg *config.Config, opts cliproxyexecutor.Options) bool {
	if cfg != nil && cfg.DisableImageGeneration != config.DisableImageGenerationOff {
		return false
	}
	return isV2ResponsesRequestPath(helps.PayloadRequestPath(opts))
}

func isV2ResponsesRequestPath(path string) bool {
	path = strings.TrimSpace(path)
	if path == "" {
		return false
	}
	return path == "/v2/responses" || strings.HasSuffix(path, "/v2/responses")
}
