...
func TestShouldAutoInjectImageGenerationToolOnlyForV2Responses(t *testing.T) {
	testCases := []struct {
		name string
		cfg  *config.Config
		path string
		...
		{name: "v2 responses", cfg: &config.Config{}, path: "/v2/responses", want: true},
		{name: "prefixed v2 responses", cfg: &config.Config{}, path: "/api/provider/openai/v2/responses", want: true},
		{name: "v1 responses", cfg: &config.Config{}, path: "/v1/responses", want: false},
		{name: "v1 images", cfg: &config.Config{}, path: "/v1/images/generations", want: false},
		{name: "empty path", cfg: &config.Config{}, path: "", want: false},
		{name: "disabled", cfg: &config.Config{SDKConfig: config.SDKConfig{DisableImageGeneration: config.DisableImageGenerationAll}}, path: "/v2/responses", want: false},
	}

	for _, tc := range testCases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			opts := cliproxyexecutor.Options{Metadata: map[string]any{cliproxyexecutor.RequestPathMetadataKey: tc.path}}
			if got := shouldAutoInjectImageGenerationTool(tc.cfg, opts); got != tc.want {
				t.Fatalf("shouldAutoInjectImageGenerationTool = %t, want %t", got, tc.want)
			}
		})
	}
}
