...
func TestApplyPayloadConfigWithRequest_LegacyV1ResponsesRemovesImageGenerationAfterPayloadOverride(t *testing.T) {
	cfg := &config.Config{
		Payload: config.PayloadConfig{
			OverrideRaw: []config.PayloadRule{
				{
					Models: []config.PayloadModelRule{
						{Name: "gpt-5.4", Protocol: "codex", FromProtocol: "openai-response"},
					},
					Params: map[string]any{
						"tools":       `[{"type":"image_generation"},{"type":"function","name":"f1"}]`,
						"tool_choice": `{"type":"image_generation"}`,
	...
	out := ApplyPayloadConfigWithRequest(cfg, "gpt-5.4", "codex", "openai-response", "", payload, nil, "", "/v1/responses", nil)

	tools := gjson.GetBytes(out, "tools")
	if !tools.IsArray() {
		t.Fatalf("expected tools array, got %v; payload=%s", tools.Type, string(out))
	}
	arr := tools.Array()
	if len(arr) != 1 || arr[0].Get("type").String() != "function" {
		t.Fatalf("expected only function tool after legacy v1 filtering, got %s", tools.Raw)
	}
	if gjson.GetBytes(out, "tool_choice").Exists() {
		t.Fatalf("expected image_generation tool_choice to be removed, payload=%s", string(out))
	}
}
