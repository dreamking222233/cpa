...
func RecordAPIRequest(ctx context.Context, cfg *config.Config, info UpstreamRequestLog) {
	...
	proxyURL := strings.TrimSpace(info.ProxyURL)
	if proxyURL == "" {
		proxyURL = ResolveProxyURL(ctx, cfg, nil)
	}
	if proxyURL != "" {
		builder.WriteString(fmt.Sprintf("Proxy: %s\n", proxyutil.Redact(proxyURL)))
	} else {
		builder.WriteString("Proxy: <none>\n")
	}
	builder.WriteString("\nHeaders:\n")
