...
func (h *Handler) persistAuthRecordIfNeeded(ctx context.Context, auth *coreauth.Auth) error {
	if auth == nil || auth.Metadata == nil {
		return nil
	}
	if auth.Attributes != nil {
...
func (h *Handler) PatchAuthFileStatus(c *gin.Context) {
	...
	if err := h.persistAuthRecordIfNeeded(ctx, targetAuth); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("failed to persist auth: %v", err)})
		return
	}
...
// PatchAuthFilesStatusBatch toggles the disabled state of multiple auth files.
func (h *Handler) PatchAuthFilesStatusBatch(c *gin.Context) {
	if h.authManager == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "core auth manager unavailable"})
		return
	}
	...
	for _, name := range names {
		...
		targetAuth.UpdatedAt = time.Now()
		if err := h.persistAuthRecordIfNeeded(ctx, targetAuth); err != nil {
			failed = append(failed, gin.H{"name": name, "error": fmt.Sprintf("failed to persist auth: %v", err)})
			continue
		}
		if _, err := h.authManager.Update(ctx, targetAuth); err != nil {
