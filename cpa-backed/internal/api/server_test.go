...
func TestV2ResponsesRoutesAreRegistered(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "responses websocket", method: http.MethodGet, path: "/v2/responses", handlerContains: ".ResponsesWebsocket"},
		{name: "responses post", method: http.MethodPost, path: "/v2/responses", handlerContains: ".Responses"},
		{name: "responses compact", method: http.MethodPost, path: "/v2/responses/compact", handlerContains: ".Compact"},
	}

	for _, tc := range testCases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			assertRouteHandlerContains(t, server, tc.method, tc.path, tc.handlerContains)
...
func assertRouteHandlerContains(t *testing.T, server *Server, method string, path string, want string) {
	t.Helper()
	...
	for _, route := range server.engine.Routes() {
		...
		if !strings.Contains(route.Handler, want) {
			t.Fatalf("%s %s handler = %q, want containing %q", method, path, route.Handler, want)
		}
		return
	}
