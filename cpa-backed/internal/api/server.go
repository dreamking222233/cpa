...
func (s *Server) setupRoutes() {
	...
	s.engine.GET("/proxy-pool.html", s.serveProxyPoolPage)
	...
	v1.Use(AuthMiddleware(s.accessManager))
	{
		...
		registerOpenAIResponsesRoutes(v1, openaiResponsesHandlers)
	}

	// OpenAI Responses v2 routes. These are intended for newer Responses clients,
	// including GPT Image 2 calls through the image_generation tool.
	v2 := s.engine.Group("/v2")
	v2.Use(AuthMiddleware(s.accessManager))
	{
		registerOpenAIResponsesRoutes(v2, openaiResponsesHandlers)
	}
	...
	{
		registerOpenAIResponsesRoutes(codexDirect, openaiResponsesHandlers)
	}
...
func registerOpenAIResponsesRoutes(group *gin.RouterGroup, handler *openai.OpenAIResponsesAPIHandler) {
	group.GET("/responses", handler.ResponsesWebsocket)
	group.POST("/responses", handler.Responses)
	group.POST("/responses/compact", handler.Compact)
}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/proxy-pool/request-logs", s.mgmt.GetProxyPoolRequestLogs)
		mgmt.GET("/openai-request-logs", s.mgmt.GetOpenAIRequestLogs)
		...
		mgmt.PATCH("/auth-files/status/batch", s.mgmt.PatchAuthFilesStatusBatch)
...
func (s *Server) serveManagementControlPanel(c *gin.Context) {
	...
	if _, err := os.Stat(filePath); err != nil {
		if os.IsNotExist(err) {
			...
			if !managementasset.EnsureLatestManagementHTML(context.Background(), managementasset.StaticDir(s.configFilePath), &cfg.SDKConfig, cfg.RemoteManagement.PanelGitHubRepository) {
				c.AbortWithStatus(http.StatusNotFound)
				return
			}
		} else {
	...
	c.Data(http.StatusOK, "text/html; charset=utf-8", injectManagementAuthFilesBulkHTML(data))
}
