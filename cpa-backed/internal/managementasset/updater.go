...
func runAutoUpdater(ctx context.Context) {
	...
	runOnce := func() {
		...
		EnsureLatestManagementHTML(ctx, staticDir, &cfg.SDKConfig, cfg.RemoteManagement.PanelGitHubRepository)
	}
...
func newHTTPClient(proxyCfg *sdkconfig.SDKConfig) *http.Client {
	client := &http.Client{Timeout: 15 * time.Second}
	util.SetProxy(proxyCfg, client)
...
func EnsureLatestManagementHTML(ctx context.Context, staticDir string, proxyCfg *sdkconfig.SDKConfig, panelRepository string) bool {
	if ctx == nil {
		ctx = context.Background()
	}
