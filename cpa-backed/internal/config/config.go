...
// SanitizeProxyPool trims proxy pool entries and drops entries without a URL.
func (cfg *Config) SanitizeProxyPool() {
	...
	for i := range cfg.ProxyPool {
		...
		if errParse != nil {
			log.WithError(errParse).WithField("proxy", proxyutil.Redact(entry.URL)).Warn("proxy-pool entry dropped: invalid proxy URL")
			continue
		}
		if setting.Mode != proxyutil.ModeProxy {
			log.WithField("proxy", entry.URL).Warn("proxy-pool entry dropped: proxy pool entries must be concrete proxy URLs")
			continue
		}
...
// SanitizeProxyPoolStrategy normalizes proxy-pool selection settings.
func (cfg *Config) SanitizeProxyPoolStrategy() {
	if cfg == nil {
		return
	}

	cfg.ProxyPoolStrategy.Mode = strings.ToLower(strings.TrimSpace(cfg.ProxyPoolStrategy.Mode))
	switch cfg.ProxyPoolStrategy.Mode {
	case "", ProxyPoolModeRandom:
		cfg.ProxyPoolStrategy.Mode = ProxyPoolModeRandom
	case ProxyPoolModeRotate:
		// keep as-is
	default:
		log.WithField("mode", cfg.ProxyPoolStrategy.Mode).Warn("invalid proxy-pool strategy mode; falling back to random")
		cfg.ProxyPoolStrategy.Mode = ProxyPoolModeRandom
	}

	if cfg.ProxyPoolStrategy.RequestsPerProxy <= 0 {
		cfg.ProxyPoolStrategy.RequestsPerProxy = DefaultProxyPoolRequestsPerProxy
	}
}
