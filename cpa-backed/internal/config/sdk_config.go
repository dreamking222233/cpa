...
type SDKConfig struct {
	...
	// ProxyPool is an optional list of outbound proxies. When auth-level proxy
	// settings are absent, one enabled proxy is selected according to ProxyPoolStrategy.
	ProxyPool []ProxyPoolEntry `yaml:"proxy-pool,omitempty" json:"proxy-pool,omitempty"`

	// ProxyPoolStrategy controls how enabled proxy-pool entries are selected.
	ProxyPoolStrategy ProxyPoolStrategy `yaml:"proxy-pool-strategy,omitempty" json:"proxy-pool-strategy,omitempty"`
...
// ProxyPoolEntry defines one outbound proxy candidate.
...
const (
	ProxyPoolModeRandom = "random"
	ProxyPoolModeRotate = "rotate"

	DefaultProxyPoolRequestsPerProxy = 100
)

// ProxyPoolStrategy defines how enabled proxy-pool entries are selected.
type ProxyPoolStrategy struct {
	Mode             string `yaml:"mode,omitempty" json:"mode,omitempty"`
	RequestsPerProxy int    `yaml:"requests-per-proxy,omitempty" json:"requests-per-proxy,omitempty"`
}
