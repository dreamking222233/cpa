...
// ResponsesWebsocket handles websocket requests for /v1/responses and /v2/responses.
...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	for {
		...
		if isLegacyResponsesEndpoint(c) && responsesPayloadRequestsImageGeneration(payload) {
			errMsg := &interfaces.ErrorMessage{StatusCode: http.StatusBadRequest, Error: fmt.Errorf("%s", imageGenerationRequiresV2Message)}
