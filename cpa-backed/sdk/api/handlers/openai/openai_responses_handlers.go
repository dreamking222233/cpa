...
const imageGenerationRequiresV2Message = "当前v1/responses接口不支持图片生成"
...
// Responses handles the /v1/responses and /v2/responses endpoints.
...
func (h *OpenAIResponsesAPIHandler) Responses(c *gin.Context) {
	...
	if rejectLegacyResponsesImageGenerationRequest(c, rawJSON) {
		return
	}
...
func rejectLegacyResponsesImageGenerationRequest(c *gin.Context, rawJSON []byte) bool {
	if !isLegacyResponsesEndpoint(c) || !responsesPayloadRequestsImageGeneration(rawJSON) {
		return false
	}
	writeImageGenerationRequiresV2Response(c)
	return true
}

func isLegacyResponsesEndpoint(c *gin.Context) bool {
...
func responsesPayloadRequestsImageGeneration(rawJSON []byte) bool {
	if isDefaultImagesToolModel(gjson.GetBytes(rawJSON, "model").String()) {
		return true
	}
	...
	if strings.EqualFold(strings.TrimSpace(toolChoice.Get("type").String()), "image_generation") {
		return true
	}
	return strings.EqualFold(strings.TrimSpace(toolChoice.Get("name").String()), "image_generation")
}

func writeImageGenerationRequiresV2Response(c *gin.Context) {
	c.JSON(http.StatusBadRequest, handlers.ErrorResponse{
		Error: handlers.ErrorDetail{
			Message: imageGenerationRequiresV2Message,
			Type:    "invalid_request_error",
			Code:    "image_generation_requires_v2_responses",
		},
	})
}
