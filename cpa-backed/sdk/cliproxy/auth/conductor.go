...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		execCtx := contextWithSelectedProxyURL(ctx, auth)
...
func contextWithSelectedProxyURL(ctx context.Context, auth *Auth) context.Context {
	if ctx == nil || auth == nil {
		return ctx
	}
	...
	return context.WithValue(ctx, "cliproxy.selected_proxy_url", proxyURL)
}
...
func (m *Manager) tryAntigravityCreditsExecute(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, bool) {
	...
	for _, c := range candidates {
		...
		creditsCtx := contextWithSelectedProxyURL(WithAntigravityCredits(ctx), c.auth)
