...
type AuthFileBatchStatusResult = {
  status: string;
  disabled: boolean;
  updated: number;
  files: AuthFileBatchStatusFile[];
  failed: AuthFileBatchFailure[];
};
...
const normalizeBatchStatusFiles = (value: unknown): AuthFileBatchStatusFile[] => {
  if (!Array.isArray(value)) return [];
  return value.reduce<AuthFileBatchStatusFile[]>((result, item) => {
    if (!item || typeof item !== 'object') return result;
    const entry = item as Record<string, unknown>;
    const name = String(entry.name ?? '').trim();
...
const normalizeBatchStatusResponse = (
  payload: AuthFileBatchStatusResponse | undefined,
  requestedNames: string[],
  disabled: boolean
): AuthFileBatchStatusResult => {
  const failed = normalizeBatchFailures(payload?.failed);
  const files = normalizeBatchStatusFiles(payload?.files);
  const updated =
  ...
  if (normalizedFiles.length === 0 && updated > 0) {
    const failedNames = new Set(
      failed.map((entry) => entry.name.trim()).filter(Boolean)
    );
    normalizedFiles = requestedNames
  ...
  return {
    status: typeof payload?.status === 'string' ? payload.status : failed.length > 0 ? 'partial' : 'ok',
    disabled: payload?.disabled === true || disabled,
    updated,
...
export const authFilesApi = {
  ...
  async setStatusBatch(names: string[], disabled: boolean): Promise<AuthFileBatchStatusResult> {
    const requestedNames = normalizeRequestedAuthFileNames(names);
    if (requestedNames.length === 0) {
      return { status: 'ok', disabled, updated: 0, files: [], failed: [] };
    }
    ...
    return normalizeBatchStatusResponse(payload, requestedNames, disabled);
  },
